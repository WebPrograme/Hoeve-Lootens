...
if (window.location.pathname == '/pages/shop.html') {
	postRequest('init/public', {}).then((res) => {
		if (res.status == 200) {
			...
			if (new Date().getTime() >= new Date('2023-08-31T16:00:00').getTime()) {
				shop.innerHTML = `
                    <div class="text-center">
                        <h2>De inschrijvingen zijn gesloten!</h2>
                        <p>De voorverkoop is gesloten. Je kan wel nog voor de KetnetBand en de After Work Party tickets kopen aan de kassa.</p>
                    </div>
                `;
				...
				if (data[key]['Available Places'] > 0) {
					...
					if (data[key]['Type'] == 'Activity' || data[key]['Type'] == 'QR' || data[key]['Type'] == 'Quiz') {
						card.innerHTML =
							...
							data[key]['Date'] +
							`</p>
                            <a class="btn btn-primary btn-primary-sm btn-shop-add" data-value="` +
							key +
							`" data-target-modal="modal-signup">Tickets</a>
                        `;
			...
			document.querySelectorAll('.btn-shop-add').forEach((el) => {
				el.addEventListener('click', (e) => {
					...
					modal.querySelector('.shop-price').innerHTML =
						data[e.target.getAttribute('data-value')]['Type'] == 'QR'
							? '€' + data[e.target.getAttribute('data-value')]['Price'] + '/pp'
							: '€' + data[e.target.getAttribute('data-value')]['Price'];
					...
					if (data[e.target.getAttribute('data-value')]['Type'] == 'Quiz') {
						...
						if (modal.querySelector('.shop-input[name="Personen"]') != null) {
							...
							personsContainer.innerHTML =
								...
								data[e.target.getAttribute('data-value')]['Available Places'] +
								`">
                            `;
